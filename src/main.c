...
void	move(t_nfo *nfo, int new_x, int new_y, int d)
{
	...
	if (nfo->map->grid[new_y][new_x] != '1')
	{
		...
		if (nfo->map->grid[new_y][new_x] == 'C')
		{
			...
			mlx_put_image_to_window(nfo->mlx, nfo->windw, nfo->gfx[0], TILE_SIZE * new_x, TILE_SIZE * new_y);
			if (nfo->hero->nb_collected == nfo->map->nb_collectibles)
				mlx_put_image_to_window(nfo->mlx, nfo->windw, nfo->gfx[2], TILE_SIZE * nfo->map->exit_x, TILE_SIZE * nfo->map->exit_y);
		}
		...
		{
			nfo->map->grid[y][x] = '0';
			mlx_put_image_to_window(nfo->mlx, nfo->windw, nfo->gfx[0], TILE_SIZE * x, TILE_SIZE * y);
		}
		else
		{
			mlx_put_image_to_window(nfo->mlx, nfo->windw, nfo->gfx[3], TILE_SIZE * x, TILE_SIZE * y);
		}
		mlx_put_image_to_window(nfo->mlx, nfo->windw, nfo->gfx[d], TILE_SIZE * new_x, TILE_SIZE * new_y);
		nfo->hero->x = new_x;
		nfo->hero->y = new_y;
	}
}
...
void	load_textures(t_nfo *nfo)
{
	...
	nfo->gfx[5] = mlx_xpm_file_to_image(nfo->mlx, TXhero_R, &size, &size);
	nfo->gfx[6] = mlx_xpm_file_to_image(nfo->mlx, TXhero_L, &size, &size);
	nfo->gfx[7] = mlx_xpm_file_to_image(nfo->mlx, TXhero_D, &size, &size);
	nfo->gfx[8] = mlx_xpm_file_to_image(nfo->mlx, TXhero_U, &size, &size);
}
...
void	blit_image(t_nfo *nfo)
{
	...
	while (y < nfo->map->height)
	{
		...
		while (x < nfo->map->width)
		{
			...
			c = nfo->map->grid[y][x];
			if (c == '1')
				mlx_put_image_to_window(nfo->mlx, nfo->windw, nfo->gfx[1], TILE_SIZE * x, TILE_SIZE * y);
			else if (c == 'P')
				mlx_put_image_to_window(nfo->mlx, nfo->windw, nfo->gfx[7], TILE_SIZE * x, TILE_SIZE * y);
			else if (c == 'E')
				mlx_put_image_to_window(nfo->mlx, nfo->windw, nfo->gfx[3], TILE_SIZE * x, TILE_SIZE * y);
			else if (c == 'C')
				mlx_put_image_to_window(nfo->mlx, nfo->windw, nfo->gfx[4], TILE_SIZE * x, TILE_SIZE * y);
			x++;
		}
		y++;
	}
...
int	main(int argc, char *argv[])
{
	...
	nfo.winw = nfo.map->width * TILE_SIZE;
	nfo.winh = nfo.map->height * TILE_SIZE;
	nfo.mlx = mlx_init();
	if (!nfo.mlx)
		return (1);
	...
	system("leaks ./so_long maps/19.ber");
	return (0);
}
