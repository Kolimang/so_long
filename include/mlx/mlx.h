...
**  return void *0 if failed
*/
int	mlx_clear_window(void *mlx, void *windw);
int	mlx_pixel_put(void *mlx, void *windw, int x, int y, int color);
/*
...
**  return void *0 if failed
*/
char	*mlx_get_nfo_addr(void *img_ptr, int *bits_per_pixel,
			   int *size_line, int *endian);
/*
**  endian : 0 = sever X is little endian, 1 = big endian
**  endian : useless on macos, client and graphical framework have the same endian
*/
int	mlx_put_image_to_window(void *mlx, void *windw, void *img_ptr,
				int x, int y);
...
int	mlx_mouse_hook (void *windw, int (*funct_ptr)(), void *param);
int	mlx_key_hook (void *windw, int (*funct_ptr)(), void *param);
int	mlx_expose_hook (void *windw, int (*funct_ptr)(), void *param);
...
int	mlx_string_put(void *mlx, void *windw, int x, int y, int color,
		       char *string);
void	*mlx_xpm_to_image(void *mlx, char **xpm_nfo,
			  int *width, int *height);
void	*mlx_xpm_file_to_image(void *mlx, char *filename,
			       int *width, int *height);
void    *mlx_png_file_to_image(void *mlx, char *file, int *width, int *height);

int	mlx_destroy_window(void *mlx, void *windw);
...
int	mlx_hook(void *windw, int x_event, int x_mask,
                 int (*funct)(), void *param);

int     mlx_mouse_hide();
int     mlx_mouse_show();
int     mlx_mouse_move(void *windw, int x, int y);
int     mlx_mouse_get_pos(void *windw, int *x, int *y);
