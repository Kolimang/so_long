...
- (int)	pixel_management
{
  ...
  glBuffernfo(GL_ARRAY_BUFFER, sizeof(pixel_vertexes), pixel_vertexes, GL_DYNAMIC_DRAW); // 4 points buff
...
void mlx_clear_window(mlx_t *mlx, mlx_win_list_t *windw)
{
  [(id)(windw->winid) selectGLContext];
  [(id)(windw->winid) clearWin];
  windw->nb_flush ++;
}


void mlx_expose_hook(mlx_win_list_t *windw, int (*funct_ptr)(), void *param)
{
  [(id)(windw->winid) setEvent:12 andFunc:funct_ptr andParam:param];
}

void mlx_key_hook(mlx_win_list_t *windw, int (*funct_ptr)(), void *param)
{
  [(id)(windw->winid) setEvent:3 andFunc:funct_ptr andParam:param];
}

void mlx_mouse_hook(mlx_win_list_t *windw, int (*funct_ptr)(), void *param)
{
  [(id)(windw->winid) setEvent:4 andFunc:funct_ptr andParam:param];
}

void mlx_hook(mlx_win_list_t *windw, int x_event, int x_mask, int (*funct_ptr)(), void *param)
{
  [(id)(windw->winid) setEvent:x_event andFunc:funct_ptr andParam:param];
}
